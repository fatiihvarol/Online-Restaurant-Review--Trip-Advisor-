...
if (isset($_GET["id"])) {
    $id = $_GET["id"];
    $sql = $sql = "SELECT * FROM foods where FoodId=$id";
    $result1 = $conn->query($sql);
    $row1 = $result1->fetch();

}
$sql2 = $sql2 = "SELECT RestaurantId FROM foods where FoodId=$id";
...
if (array_key_exists('button',$_POST)) {
    ...
    $sql = "UPDATE foods " . "SET FoodName='$foodname',FoodDescription='$fooddesc',FoodPrice='$foodprice',isHot='$isHotTemp'" . "WHERE FoodId=$id ";
    $result2 = $conn->query($sql);
    $log = $conn->prepare('INSERT INTO logs (Message) VALUES (?)');
    $log->execute([
      
      "UPDATE foods " . "SET FoodName='$foodname',FoodDescription='$fooddesc',FoodPrice='$foodprice',isHot='$isHotTemp'" . "WHERE FoodId=$id"
...
<body>
    ...
    <body>
        ...
        <header>
            ...
            <nav>
                <ul>
                    <li><a href="TakeBeverages.php" class="cta">Add Beverage</a></li>
        ...
        <form1 class="form1" method="POST" action="#">
            <form method="post">
                <div class="form-field">
                    <input type="text" value= <?php echo $row1['FoodName'] ?> name="FoodName" required />
                </div>

                <div class="form-field">
                    <input type="text" value=<?php echo $row1['FoodDescription'] ?>  name="FoodDescription" required />
                </div>



                <div class="form-field">
                    <input type="number" value= <?php echo $row1['FoodPrice'] ?> name="FoodPrice" required />
                </div>
                ...
                <form action="" method="post">
                    ...
                    <form>
                        ...
                        <div class="form-field">
                            <button class="btn"  name="button">Edit Food</button>
                        </div>
                    <form>
                <form>
            <form>
